...
class ToolChecker:
        ...
        try:
            ...
            for tool_id, tool_info in registry.TOOLS.items():
                try:
                    ...
                    print(f"检查: {tool_id} - {'[OK]' if status.installed else '[FAIL]'}")
    ...
    def generate_report(self, check_results: Dict[str, Any]) -> str:
        ...
        report_lines.append("ClawAI 工具安装状态检查报告")
        report_lines.append("=" * 80)
        ...
        if self.system == "windows":
            ...
            report_lines.append("  3. 或将工具添加到 PATH 环境变量")
        elif self.system == "linux":
            report_lines.append("  1. 使用 apt, yum 或 pacman 安装系统包")
            report_lines.append("  2. 使用 pip 或 go install 安装Python/Go工具")
            report_lines.append("  3. 从 GitHub 源码编译安装")
    ...
    def save_results(self, check_results: Dict[str, Any], filename: str = "tool_check_results.json"):
        """保存检查结果到文件"""
        try:
    ...
    def print_detailed_list(self, check_results: Dict[str, Any]):
        ...
        for category, category_tools in sorted(categories.items()):
            ...
            for tool in sorted(category_tools, key=lambda x: x["tool_id"]):
                status = "[OK]" if tool["installed"] else "[FAIL]"
                version_info = f" ({tool['version']})" if tool["version"] else ""
                print(f"  {status} {tool['tool_id']:20} - {tool['name']}{version_info}")
...
def main():
    ...
    if summary["install_rate"] < 50:
        print(f"\n[WARN] 工具安装率较低 ({summary['install_rate']:.1f}%)，建议安装核心工具:")
        print("   1. nmap - 端口扫描器")
        print("   2. nuclei - 漏洞扫描器") 
        print("   3. dirsearch - 目录爆破工具")
        print("   4. httpx - HTTP探测工具")
