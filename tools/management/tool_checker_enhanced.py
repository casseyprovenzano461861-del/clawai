...
class EnhancedToolChecker:
    ...
    def __init__(self):
        ...
        self.tools_directory = "C:\\Tools"
        self.local_tools_directory = "e:\\ClawAI\\工具"  # 新添加的本地工具目录
    ...
    def check_tool(self, tool_id: str, tool_info: Dict) -> ToolStatus:
        ...
        if not installed:
            ...
            if custom_path:
                # 检查是否在本地工具目录
                if self.local_tools_directory and custom_path.startswith(self.local_tools_directory):
                    install_guide = f"[OK] 已安装于: {executable_path} (在本地工具目录)"
                elif custom_path.startswith(self.tools_directory):
                    install_guide = f"[OK] 已安装于: {executable_path} (在C:\\Tools目录)"
                else:
                    install_guide = f"[OK] 已安装于: {executable_path}"
            else:
                install_guide = f"[OK] 已安装于: {executable_path}"
    ...
    def _find_in_tools_directory(self, tool_id: str) -> Optional[str]:
        """在C:\Tools目录或本地工具目录中查找工具"""
        # 首先检查本地工具目录 (e:\ClawAI\工具)
        if os.path.exists(self.local_tools_directory):
            # 本地工具目录映射
            local_tool_paths = {
                "sqlmap": [r"sqlmap\sqlmap.py"],
                "dirsearch": [r"dirsearch\dirsearch.py"],
                "wafw00f": [r"wafw00f\wafw00f\main.py"],
                "commix": [r"commix\commix.py"],
                "joomscan": [r"joomscan\joomscan.pl"],
                "cmsmap": [r"cmsmap\cmsmap.py"],
                "xsstrike": [r"XSStrike\xsstrike.py"],
                "theharvester": [r"theHarvester\theHarvester.py"],
                "testssl": [r"testssl.sh\testssl.sh"],
            }
            
            if tool_id in local_tool_paths:
                for relative_path in local_tool_paths[tool_id]:
        ...
        try:
            ...
            for tool_id, tool_info in registry.TOOLS.items():
                try:
                    ...
                    print(f"检查: {tool_id} - {'✅' if status.installed else '❌'}")
...
def main():
    """主函数"""
    print("[SEARCH] ClawAI 增强版工具安装检查器")
    ...
    if summary["install_rate"] < 50:
        print(f"\n⚠️  工具安装率较低 ({summary['install_rate']:.1f}%)，建议:")
