...
#!/usr/bin/env python3
"""
创建不带BOM的UTF-8编码批处理文件
"""

import os
import sys

def create_utf8_no_bom():
    """创建不带BOM的UTF-8编码批处理文件"""
    content = '''@echo off
...
'''
    ...
    # 备份原文件
    if os.path.exists('start_utf8_nobom.bat'):
        backup_count = 1
        while os.path.exists(f'start_utf8_nobom.bat.backup{backup_count}'):
            backup_count += 1
        os.rename('start_utf8_nobom.bat', f'start_utf8_nobom.bat.backup{backup_count}')
    
    # 保存为UTF-8 without BOM
    try:
        with open('start_utf8_nobom.bat', 'wb') as f:
            f.write(content.encode('utf-8'))
        print("UTF-8 without BOM版本创建成功: start_utf8_nobom.bat")
        
        # 检查文件编码
        with open('start_utf8_nobom.bat', 'rb') as f:
            first_bytes = f.read(10)
            if first_bytes[:3] == b'\xef\xbb\xbf':
                print("警告: 文件意外包含BOM标记")
            else:
                print("验证: 文件不包含BOM标记")
...
'''
    ...
    try:
        with open('test_utf8_nobom.bat', 'wb') as f:
            f.write(content.encode('utf-8'))
        print("UTF-8 without BOM测试文件创建成功: test_utf8_nobom.bat")
        return True
    except Exception as e:
        print(f"创建测试文件失败: {e}")
        return False
...
def main():
    """主函数"""
    print("创建UTF-8 without BOM编码批处理文件")
    print("=" * 60)
    
    # 创建测试文件
    if create_simple_test():
        print("\n简单测试文件创建成功")
        print("请运行 'test_utf8_nobom.bat' 测试UTF-8 without BOM支持")
    
    # 创建完整版本
    if create_utf8_no_bom():
        print("\n完整UTF-8 without BOM版本创建成功")
        print("请运行 'start_utf8_nobom.bat' 测试完整功能")
    
    print("\n" + "=" * 60)
    print("重要说明:")
    print("1. 这些文件使用UTF-8 without BOM编码")
    print("2. 文件开头设置了 'chcp 65001' 命令来启用UTF-8代码页")
    print("3. 如果仍有问题，可能是:")
    print("   - 系统不支持UTF-8代码页")
    print("   - 控制台字体不支持中文")
    print("4. 备用方案:")
    print("   - 英文版: 'start_english_final.bat' (推荐)")
    print("   - GBK版: 'start.bat'")
