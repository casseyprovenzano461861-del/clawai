...
class UnifiedExecutor(BaseExecutor):
    ...
    def __init__(self,
                 ...
                 use_tool_executor_api: bool = True):
        """
        初始化统一执行器
        ...
        Args:
            ...
            use_tool_executor_api: 是否使用工具执行器API（默认为True）
        """
        ...
        self.use_tool_executor_api = use_tool_executor_api
        self.tool_executor_url = "http://localhost:8082"
    ...
    def _load_tools_config(self) -> Dict[str, Dict[str, Any]]:
        ...
        def get_tool_path(config_name: str, default_cmd: str) -> str:
            ...
            # 3. 检查config.py中的配置路径
            try:
                config_path = getattr(config, config_name, None)
                if config_path and os.path.exists(config_path):
                    self.logger.info(f"config配置中找到工具 {tool_name}: {config_path}")
                    return config_path
            except Exception as e:
                self.logger.debug(f"检查config配置路径失败: {e}")
    ...
    def _execute_via_tool_executor(self, tool_name: str, target: str, options: Dict) -> Dict[str, Any]:
        """
        通过工具执行器API执行工具
        ...
        Raises:
            RuntimeError: 如果工具执行器API调用失败
        """
        if not self.use_tool_executor_api:
            raise RuntimeError("工具执行器API使用已禁用")
        ...
        try:
            ...
            self.logger.info(f"通过工具执行器API执行工具: {tool_name}, 目标: {target}")

            # 发送POST请求到工具执行器API
            response = requests.post(
                f"{self.tool_executor_url}/api/v1/tools/execute",
                json=request_data,
                timeout=10
            )

            if response.status_code != 200:
                raise RuntimeError(f"工具执行器API返回错误状态码: {response.status_code}, 响应: {response.text}")

            result = response.json()
            task_id = result.get("task_id")

            if not task_id:
                raise RuntimeError(f"工具执行器API响应中缺少task_id: {result}")

            # 轮询任务状态
            max_polling_time = 600  # 最大轮询时间10分钟
            polling_interval = 2    # 轮询间隔2秒
            start_time = time.time()

            while time.time() - start_time < max_polling_time:
                # 获取任务状态
                status_response = requests.get(
                    f"{self.tool_executor_url}/api/v1/tasks/{task_id}",
                    timeout=5
                )
                ...
                if status_response.status_code == 200:
                    ...
                    if task_status == "completed":
                        ...
                        standardized_result = {
                            ...
                            "execution_mode": "real",
                            "method": "tool_executor_api"
                        }
                        self.logger.info(f"工具执行器API执行成功: {tool_name}, 任务ID: {task_id}")
                        return standardized_result
                    elif task_status == "failed":
                        error_msg = task_data.get("error", "任务执行失败")
                        raise RuntimeError(f"工具执行器任务失败: {error_msg}")
                    ...
                    self.logger.warning(f"获取任务状态失败: {status_response.status_code}")

                time.sleep(polling_interval)

            # 超时
            raise RuntimeError(f"工具执行器任务超时，任务ID: {task_id}")

        except requests.exceptions.RequestException as e:
            raise RuntimeError(f"工具执行器API请求失败: {str(e)}")
        except Exception as e:
            raise RuntimeError(f"工具执行器API调用异常: {str(e)}")

    def _execute_single_tool(self, tool_name: str, target: str, options: Dict) -> Dict[str, Any]:
        """执行单个工具（真实执行）"""
        # 首先尝试使用工具执行器API（如果启用）
        if self.use_tool_executor_api:
            try:
                self.logger.info(f"尝试通过工具执行器API执行工具: {tool_name}")
                return self._execute_via_tool_executor(tool_name, target, options)
            except Exception as e:
                self.logger.warning(f"工具执行器API调用失败，回退到标准执行: {str(e)}")
                # 继续执行标准执行逻辑
    ...
    def _execute_sqlmap_real(self, target: str, options: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取sqlmap路径
            sqlmap_path = self.get_tool_path("SQLMAP_PATH", "sqlmap")

            # 对于DVWA，测试SQL注入漏洞
            # 构建基本命令
            if sqlmap_path.endswith(".py"):
                # 如果是Python脚本，使用python执行
                cmd = ["python", sqlmap_path, "-u", target,
                       "--non-interactive",  # 非交互模式
                       "--level=1", "--risk=1",  # 低风险级别
                       "--timeout=30",
                       "--flush-session"]
            else:
                # 如果是可执行文件
                cmd = [sqlmap_path, "-u", target,
                       "--non-interactive",  # 非交互模式
                       "--level=1", "--risk=1",  # 低风险级别
                       "--timeout=30",
                       "--flush-session"]
            ...
            self.logger.info(f"执行sqlmap命令: {' '.join(cmd)}")
            result = subprocess.run(
                cmd,
                capture_output=True,
                text=True,
            ...
            return {
                ...
                "execution_mode": "real",
                "success": result.returncode == 0 or len(injections) > 0
            }

        except Exception as e:
            raise RuntimeError(f"sqlmap执行错误: {str(e)}")
